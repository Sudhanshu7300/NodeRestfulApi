...
exports.signup = (req,res)=>{
   ...
   if(error){ return res.status(400).json({ message:'Something want wrong'
       ...
       if(data){  return res.status(201).json({message:'user created Succesfully..!' })
       }
         })   
        })
      }
    ...
    User.findOne({email: req.body.email})
    .exec((error,user)=>{                                                             //exexute the request going to error 
        if(error) return res.status(400).json({error})
        if(user){                                                                     //save and if a user if user exists // and verify the pass

            if(user.authenticate(req.body.password)&& user.role === 'user'){ 
       const token = jwt.sign({_id: user._id, role: user.role}, process.env.JWT_SECRET,{expiresIn: '1h' });  // token Genrate 
